...
export function coordinatorMutationSyncStatus(
  mutationId: string,
  appliedIds: readonly string[],
  offline: boolean
): { synced: boolean; queued: boolean; offline: boolean } {
  if (offline) {
    return { synced: false, queued: true, offline: true }
  }
  const synced = appliedIds.includes(mutationId)
  return { synced, queued: !synced, offline: false }
}
...
export async function commitCoordinatorMutation(
  ...
  if (offline) {
    return coordinatorMutationSyncStatus(entry.id, [], true)
  }
  const result = await flushCoordinatorOpsQueue(eventId)
  return coordinatorMutationSyncStatus(entry.id, result.appliedIds, false)
}
