...
export interface FlushCoordinatorOpsQueueResult {
  synced: number
  failed: number
  remaining: number
  appliedIds: string[]
...
/** Whether the mutation the caller just queued was applied during flush. */
export function resolveMutationSyncStatus(
  mutationId: string,
...
const flushLocks = new Map<string, Promise<FlushCoordinatorOpsQueueResult>>()

async function flushCoordinatorOpsQueueInner(
  eventId: string
): Promise<FlushCoordinatorOpsQueueResult> {
...
export async function flushCoordinatorOpsQueue(
  ...
  const inFlight = flushLocks.get(eventId)
  if (inFlight) return inFlight

  const work = flushCoordinatorOpsQueueInner(eventId).finally(() => {
    if (flushLocks.get(eventId) === work) {
      flushLocks.delete(eventId)
    }
  })
  flushLocks.set(eventId, work)
  return work
}
...
export async function commitCoordinatorMutation(
  ...
  return resolveMutationSyncStatus(entry.id, result.appliedIds)
}
