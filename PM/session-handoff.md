...
- **Requires:** `BUILD_PROVISION_PROFILE_BASE64` must be a current **App Store** distribution profile for `ca.popuphub.app` on team `6ACBDTX7T7`, embedding the same Apple Distribution cert as `BUILD_CERTIFICATE_BASE64`. If archive errors `doesn't include signing certificate`, re-export a matching App Store profile (see PM/ios-testflight.md troubleshooting).
