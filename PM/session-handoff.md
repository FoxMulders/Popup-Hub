...
- **Attempt #47 (2026-06-26 investigation):** Attached Action log (15:23 UTC) still shows Attempt #45 errors (`iOS App Development provisioning profiles` / `no devices`). `App.entitlements` on `master` already lacks `aps-environment` (fixed in `1f134bf`), so remaining risks: (1) `scripts/mobile/generate-ios-resources.mjs` overwrote entitlements with `aps-environment=development` on every `mobile:assets` run — fixed; (2) deploy workflow still copied a stale manual `BUILD_PROVISION_PROFILE_BASE64` into `~/Library/MobileDevice/Provisioning Profiles/`, blocking ASC auto-profile refresh — removed profile import; (3) archive step now clears local profiles before build and passes `CODE_SIGN_IDENTITY=Apple Distribution` to force distribution signing.
...
- **Fallbacks if archive still fails:** (1) `profile doesn't include com.apple.developer.associated-domains` → temporarily remove associated-domains entitlement; (2) ASC API key lacks profile-create permission → verify App Manager role on key.
