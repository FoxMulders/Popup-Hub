...
- **Shipped:**
  - **Migration `136_conversion_engine_external_listing.sql`** — `events.is_external_listing`, `destination_url`, `ad_campaign_status`, `ad_campaign_expires_at`; `ad_clicks_log` + dedupe index; RLS read/write split on `booth_layouts` and `booth_applications`.
...
- **Ops automation:** `scripts/seed-external-listing-market.ts`, `scripts/verify-conversion-engine.ts`, `scripts/run-conversion-engine-ops.sh`, `docs/CONVERSION_ENGINE_DEPLOY.md`, GitHub Actions `conversion-engine-ops.yml`.
- **Prod status (Jul 1 2026):** popuphub.ca build still `45dad9f`. Workflow run `28540045728` **failed** — `SUPABASE_ACCESS_TOKEN` GitHub secret not configured. Add secrets per `docs/CONVERSION_ENGINE_DEPLOY.md`, then re-run **Conversion Engine Ops** or `npm run ops:conversion-engine` locally.
...
- **Shipped (merged PR #166 @ `6675bd95`):**
...
- **Prod deploy:** Pending — run `PM\Deploy-popuphub.bat` or `npx vercel deploy --prod --yes` from authenticated machine (`vercel.json` disables git deploy on `master`).
...
- **Next:** Commit #60 profile-decode hardening → push → re-run **Deploy to TestFlight**. If it still fails after whitespace strip, re-encode both `.mobileprovision` files (`base64 -i <file>.mobileprovision | pbcopy`) and update `BUILD_PROVISION_PROFILE_BASE64` + `BUILD_WIDGET_PROVISION_PROFILE_BASE64`. Expect `ARCHIVE SUCCEEDED` → `EXPORT SUCCEEDED` → upload.
...
- **Shipped locally:**
  ...
  - **Bug fix (branch `cursor/critical-bug-investigation-35ef`):** `commitCoordinatorMutation` no longer reports `synced` when a *different* queued mutation applied in the same flush batch — prevents silent data loss on check-in/payment when stale queue items sync first. Per-event flush lock avoids duplicate concurrent sync POSTs.
...
- **Next:** Merge bug-fix PR; commit + deploy when user asks. Smoke: outdoor market booth on open lot → orange advisory; airplane mode check-in → reconnect sync; vendor print sign → scan opens vendor profile.
...
- **Attempt #48 result:** Archive still failed (run #52). Cert + API key steps OK; archive step still failing.
- **Attempt #49 fix — `App.entitlements` + `deploy.yml`:** Cleared entitlements (defer associated-domains + push); removed CLI `CODE_SIGN_IDENTITY` override — pure Automatic + ASC API key + distribution cert in keychain + `generic/platform=iOS`.
- **Attempt #49 result:** Archive still failed (run #53). Automatic signing on headless CI keeps resolving an **iOS App Development** profile during archive (no registered devices) regardless of entitlements/identity flags.
- **Attempt #50 fix — `deploy.yml` + `ExportOptions.plist`:** Switched archive to **manual distribution signing**. Re-decode `BUILD_PROVISION_PROFILE_BASE64`, extract Name/UUID via `security cms` + PlistBuddy, install to `~/Library/MobileDevice/Provisioning Profiles/<UUID>.mobileprovision`, warn if it is a device-limited (Development/Ad Hoc) profile. Archive uses `CODE_SIGN_STYLE=Manual` + `CODE_SIGN_IDENTITY="Apple Distribution"` + `PROVISIONING_PROFILE_SPECIFIER="$PROFILE_NAME"` (no `-allowProvisioningUpdates`, no API key on archive). pbxproj left Automatic so local Xcode dev flow is unchanged (CLI overrides per-build). **Also fixed latent upload bug:** `ExportOptions.plist` `destination` `export` -> `upload` so the export step actually delivers to TestFlight (was only writing an IPA to disk). Export keeps `signingStyle=automatic` + ASC API key + `-allowProvisioningUpdates`.
- **Attempt #50 result:** Archive still failed (run #54). Cert + profile install OK; archive step failed — likely pbxproj Automatic vs CLI Manual conflict and/or stale profile secret (attempt #42 cert mismatch).
...
- **Attempt #60 root cause (profile decode failure):** Run passed **Import Apple Distribution certificate** then failed in **Install App Store provisioning profiles** with `security: failed to add data to decoder: UNKNOWN (-8183(d))` + `security: problem decoding`. The bytes decoded from `BUILD_PROVISION_PROFILE_BASE64` and/or `BUILD_WIDGET_PROVISION_PROFILE_BASE64` were not parseable CMS — typically whitespace-corrupted base64 from GitHub secret paste, or the secret holds the raw file path/text instead of base64 of the `.mobileprovision`.
- **Attempt #60 fix — `.github/workflows/deploy.yml`:** Hardened `install_profile()`: strip whitespace before `base64 --decode`, fail fast with actionable `::error::` if decoded file is empty (prints size + `file` type), surface clear error if `security cms -D` cannot parse the profile.
- **Attempt #60 next:** Commit/push #60 fix → re-run. If still failing, re-download both App Store profiles from Apple portal, `base64 -i <file>.mobileprovision | pbcopy` into the matching secret (names must be `PopupHub App Store` / `PopupHub Widget App Store`).
...
- **Shipped locally:**
  ...
  - **`package.json`:** Both tests wired into `test:unit` script.
...
## Last deploy
- 2026-06-29 18:07 - Deploy via deploy-popuphub.ps1 - `feat: ship 46 session updates (Public event TypeScript CI fix; Signup role selection + questionnaire; Market detail footer clearance; Center loader logo and animation; +42 more)` (1b9070c0)
