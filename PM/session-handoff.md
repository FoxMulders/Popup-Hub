...
- **Shipped locally:**
  ...
  - **`commitCoordinatorMutation`:** Per-mutation sync status — no false success when another queued mutation flushes first; `offline` flag separates offline queue from online flush failure.
...
- **Verify:** `npx tsx lib/pwa/coordinator-ops-offline.test.ts` PASS; `npx tsx lib/shopper/public-floorplan-modes.test.ts` PASS; `npx tsx lib/wizard/wizard-autosave.test.ts` PASS; `npx tsc --noEmit` PASS.
