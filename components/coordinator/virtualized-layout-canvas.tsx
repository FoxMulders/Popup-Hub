...
export function VirtualizedLayoutCanvas({
  ...
  const updateVisible = useCallback(() => {
    ...
    const pad = cellPx * 3
    const vCol0 = Math.max(0, Math.floor((el.scrollLeft - pad) / cellPx))
    const vCol1 = Math.min(cols - 1, Math.ceil((el.scrollLeft + el.clientWidth + pad) / cellPx))
    const vRow0 = Math.max(0, Math.floor((el.scrollTop - pad) / cellPx))
    const vRow1 = Math.min(rows - 1, Math.ceil((el.scrollTop + el.clientHeight + pad) / cellPx))
  ...
  }, [cellPx, cols, rows, partitioned, quadrants])
    ...
    onGridCellPointerDown || onGridCellPointerEnter
      ? {
          onMouseDown: (event: MouseEvent) => {
            ...
            const cell = cellFromPointer(event, cols, rows, cellPx)
  ...
  const boundsClass = cn(
    'inline-block overflow-hidden rounded-lg border-2 border-stone-300 bg-canvas',
    fitToBounds ? '' : 'max-w-full'
  )

  if (!partitioned) {
    return (
      <div
        className={cn(boundsClass, className)}
        style={{ width: gridWidthPx, maxWidth: '100%' }}
      >
  ...
  return (
    <div className={className}>
      <div
        ref={scrollRef}
        className={cn(boundsClass, 'overflow-auto')}
        style={{ width: gridWidthPx, maxWidth: '100%', maxHeight: VIRTUAL_CANVAS_MAX_HEIGHT }}
      >
        <div
          className="relative"
          style={{
