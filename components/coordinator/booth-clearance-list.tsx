...
export function BoothClearanceList({
  ...
  return (
    <div className="space-y-4">
      ...
      {!requiresPhoto && (
        <div className="flex gap-3 rounded-xl border border-sage-200 bg-sage-50 p-4 text-sm text-sage-800">
          <Info className="h-5 w-5 shrink-0 text-green-600" />
          <div>
            <p className="font-semibold">Photo verification not required</p>
            <p className="mt-0.5 text-sage-700">{instructions.body}</p>
          </div>
        </div>
      )}

      {requiresPhoto && policy === 'pack_furniture' && (
        <div className="flex gap-3 rounded-xl border border-terracotta-200 bg-terracotta-50 p-4 text-sm text-terracotta-800">
          <Info className="h-5 w-5 shrink-0 text-terracotta-600" />
          <div>
      ...
      <div className="flex items-center gap-3 rounded-xl border bg-white p-4 shadow-sm">
        ...
        {apps.length > 0 && (
          <div className="ml-4 flex-1 h-2 rounded-full bg-stone-100">
            <div
      ...
      <div className="space-y-2">
          ...
          return (
            <div
              ...
              className={`rounded-xl border bg-white p-4 flex items-center gap-3 transition-colors shadow-sm ${
                app.booth_cleared ? 'bg-sage-50 border-sage-200' : ''
              }`}
            >
              <div className="shrink-0">
                {app.booth_number != null ? (
                  <div className="h-10 w-10 rounded-lg bg-harvest-100 text-harvest-700 flex items-center justify-center text-sm font-bold">
                    {app.booth_number}
                  </div>
                ) : (
                  <div className="h-10 w-10 rounded-lg bg-stone-100 text-muted-foreground flex items-center justify-center text-xs">
                    —
                  </div>
                )}
              </div>
              ...
              <div className="shrink-0 flex items-center gap-2">
                {app.booth_cleared ? (
                  ...
                  <div className="flex items-center gap-2 flex-wrap justify-end">
                    <span className="inline-flex items-center gap-1 text-xs text-harvest-600">
        ...
        {apps.length === 0 && (
          <div className="rounded-xl border border-dashed border-stone-200 py-12 text-center">
            <p className="text-sm text-muted-foreground">No approved vendors for clearance.</p>
          </div>
        )}
      </div>
    </div>
